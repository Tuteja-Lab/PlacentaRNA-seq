...
### Checking specific genes after removing outliers =====
#map transcripts to genes
mart <- biomaRt::useMart(biomart='ENSEMBL_MART_ENSEMBL',
                         dataset = "mmusculus_gene_ensembl", host="http://sep2019.archive.ensembl.org")
...
tpm3 <- tpm2
tpm3$target_id <- row.names(tpm1)
tpm3 <- tpm3[, -which(colnames(tpm3) %in% c("E7.5_1", "E9.5_6"))]

check_genes <- function(tpm, t2g, g) {
  gene_transcript <- subset(t2g$target_id, t2g$ext_gene %in% as.vector(g))
  gene <- subset(tpm, tpm$target_id %in% gene_transcript)
  gene <- gene[, -which(colnames(gene) %in% c("target_id"))]
  mean <- colMeans(gene)
  subdat <- data.frame(Value = unlist(mean), Sample = colnames(gene))
  subdat$Sample <- c(rep("E7.5", 5), rep("E8.5", 6), rep("E9.5", 5))
  
  ggplot(subdat, aes(x=Sample, y=Value, fill=Sample)) + 
    geom_boxplot() +
    scale_fill_manual(values=c("#A5B557", "#354E71", "#841F27")) +
    labs(title = as.vector(g),
         x = "Time point",
         y = "Scaled mean transcript counts (TPM)") +
    theme(plot.title = element_text(size = 15, face = "bold"),
          axis.title.x = element_text(size = 15),
          axis.title.y = element_blank(),
          axis.text.x = element_text(size = 15),
          legend.position = "none") + ylim(-1.5, 2.6)
}

pdf("dir/tbx4_afterRmOutliers.pdf", width=4, height=8)
check_genes(tpm3, t2g, "Tbx4") #e9.5
dev.off()

pdf("Z:/hhvu/Project1_2/RNA-seq/Figures/RNAseqPaper/gcm1_afterRmOutliers.pdf", width=4, height=8)
check_genes(tpm3, t2g, "Gcm1") #e9.5
dev.off()

pdf("Z:/hhvu/Project1_2/RNA-seq/Figures/RNAseqPaper/itga4_afterRmOutliers.pdf", width=4, height=8)
check_genes(tpm3, t2g, "Itga4") #e8.5
dev.off()

pdf("Z:/hhvu/Project1_2/RNA-seq/Figures/RNAseqPaper/gjb2_afterRmOutliers.pdf", width=4, height=8)
check_genes(tpm3, t2g, "Gjb2") #e9.5
dev.off()

pdf("Z:/hhvu/Project1_2/RNA-seq/Figures/RNAseqPaper/Ncor2_afterRmOutliers.pdf", width=4, height=8)
check_genes(tpm3, t2g, "Ncor2")
dev.off()


###correlation between samples after removing outlier - heatmap ====
tpm3 <- tpm2[, which(!(colnames(tpm2) %in% c("E7.5_1", "E9.5_6")))]
cor <- cor(tpm3, method = "spearman") # spearman in case there are extreme count genes
pheatmap(cor, cluster_cols = T, cluster_rows = T, clustering_method = "complete")
