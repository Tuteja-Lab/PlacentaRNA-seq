...
placenta <- read.table("Files/placentaGenes.txt", sep = '\t', skip = 1)
placenta <- inner_join(placenta, t2g[,2:3], by = c("V1" = "ext_gene"))
placenta <- distinct(placenta)

### load networks - all gene analysis ####
coordinates <- read.table("Files/STRING/e7.5/largestComponent/e8.5specific_TSS.bed", header = F, sep = "\t")
nameMap <- read.table("Files/STRING/e8.5/e8.5_string_mapping.tsv", sep = "\t", quote = "", header = F)
colnames(nameMap) <- c("queryIndex", "queryItem",	"stringId",	"preferredName",	"annotation")

network <- read.table("Files/STRING/e8.5/largestComponent/e8.5_2/e8.5_2_nodeTable.csv", sep = ",", header = T)
...
write.table(sub_coor, "Files/STRING/e8.5/largestComponent/e8.5_3/e8.5_3_TSS.bed", sep = "\t", quote = F, row.names = F)


#use GREAT for ontology analysis
#then use the next lines for GREAT filtering
#bio process terms
great <- read.table("Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_BP_all.tsv", header = F, sep = "\t", quote = "")
...
#write.table(great, "Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_BP_filtered.csv", quote = F, sep = ',', row.names = F)
write.table(great, "Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_BP_filtered.csv", quote = F, sep = ',', row.names = F)

#single KO terms
great <- read.table("Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_singleKO_all.tsv", header = F, sep = "\t", quote = "")
great <- subset(great, great$V3 <= 0.05)
great <- subset(great, great$V4 > 2)
great <- great[, -8]
colnames(great) <- c("Terms", "Hyper.Rank", "Hyper.Qval", "Fold", "Hyper.Obs.Gene.Hits", "Hyper.Total.Genes", "Hyper.Gene.Set.Cov")
#write.table(great, "Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_singleKO_filtered.csv", quote = F, sep = ',', row.names = F)
write.table(great, "Files/STRING/e8.5/largestComponent/e8.5_3/GREAT/e8.5_3_singleKO_filtered_KK.csv", quote = F, sep = ',', row.names = F)
...
save(all, file="Files/STRING/e8.5/largestComponent/e8.5_2/e8.5_2_hubGenes.rda")
save(all, file="Files/STRING/e8.5/largestComponent/e8.5_2/e8.5_2_hubGenes_KK.rda")
