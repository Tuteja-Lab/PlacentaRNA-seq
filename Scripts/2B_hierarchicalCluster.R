...
###plot the groups ====
#png("Z:/hhvu/Project1/RNA-seq/Figures/RNAseqPaper/hierarchicalClustering_groups_noMarkers.png", units="in", width=10, height=8, res=300)
#ggplot(aes(time, mean_cts_scaled), data = summary) +
#  geom_line(aes(group = transcripts), alpha = 0.5, colour = "grey77") + #grey
#  geom_line(stat = "summary", fun.y = "median", size = 1.5, colour = "brown", #brown
#            aes(group = 1)) +
#  labs(title = "Hierarchical clustering of transcripts",
#       x = "Time point",
#       y = "Scaled mean transcript counts") +
...
ncor2 <- subset(summary, summary$ext_gene == "Ncor2")

pdf("/work/LAS/geetu-lab/hhvu/project1_2/rna-seq/2B_hierarchicalClustering/hierarchicalClustering_groups_markers.pdf", width=12, height=8)
ggplot(aes(time, mean_cts_scaled), data = summary) +
  geom_line(aes(group = transcripts), alpha = 0.5, colour = "grey77") + #grey
...
#pdf("/work/LAS/geetu-lab/hhvu/project1_2/rna-seq/hclust_proteinCodingTrans_dendrogram.pdf", width=9, height=9)
#dend %>% set("branches_k_color", k = 3, c("#841F27", "#354E71",  "#A5B557")) %>% #"saddlebrown", "orange2", "darkolivegreen4")) %>% 
#  plot(main = "Hierarchical clustering", cex.lab=1.5, cex.axis=1.5, cex.main=1.5, cex.sub=1.5, leaflab = "none", xlab = "Transcripts")
#abline(h = 7.7, col = "red", lwd = 1.5)
#dev.off()