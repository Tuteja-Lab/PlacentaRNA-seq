...
coordinates <- read.table("Z:/hhvu/Project1_2/RNA-seq/4_specificGenes/e7.5specific_TSS.bed", header = F, sep = "\t")
nameMap <- read.table("Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/e7.5_string_mapping.tsv", sep = "\t", quote = "", header = F)
colnames(nameMap) <- c("queryIndex", "queryItem",	"stringId",	"preferredName",	"annotation")

network <- read.table("Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/e7.5_1_nodeTable.csv", sep = ",", header = T)
...
write.table(sub_coor, "Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/e7.5_1_TSS.bed", sep = "\t", quote = F, row.names = F)


#use GREAT for ontology analysis
#then use the next lines for GREAT filtering
#bio process terms
great <- read.table("Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/GREAT/e7.5_1_BP_all.tsv", header = F, sep = "\t", quote = "")
...
write.table(great, "Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/GREAT/e7.5_1_BP_filtered.csv", quote = F, sep = ',', row.names = F)

#single KO terms
great <- read.table("Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/GREAT/e7.5_1_singleKO_all.tsv", header = F, sep = "\t", quote = "")
great <- subset(great, great$V3 <= 0.05)
great <- subset(great, great$V4 > 2)
great <- great[, -8]
colnames(great) <- c("Terms", "Hyper.Rank", "Hyper.Qval", "Fold", "Hyper.Obs.Gene.Hits", "Hyper.Total.Genes", "Hyper.Gene.Set.Cov")
write.table(great, "Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/GREAT/e7.5_1_singleKO_filtered.csv", quote = F, sep = ',', row.names = F)


#the next part is to analyze networks and get hub genes
network <- subset(network, network$queryItem %in% coordinates$V4)

network[which(network$queryItem %in% placenta$ens_gene), "ext_gene"]
...
save(all, file="Z:/hhvu/Project1_2/RNA-seq/5A_STRING/e7.5/largestComponent/e7.5_1/e7.5_1_hubGenes.rda")
