...
allGenes <- unique(t2g[t2g$ens_gene %in% genes$name, "ext_gene"], genes2$name)
mart <- biomaRt::useMart("ensembl", host="http://sep2019.archive.ensembl.org", dataset = "mmusculus_gene_ensembl")
...
for (i in 1:10000) {
  ...
  if (length(intersect(s, placenta$external_gene_name)) > length(intersect(all, placenta$external_gene_name))) {
    t <- t+1
  }
}
t/10000
...
length(setdiff(intersect(all$Name, embryo$external_gene_name), placenta$external_gene_name))
length(setdiff(setdiff(intersect(all$Name, development$external_gene_name), placenta$external_gene_name), embryo$external_gene_name))
