...
kal_dirs <- file.path(dir, sample_id, "abundance.tsv")
=======
dir <- "Z:/hhvu/Project1_2/RNA-seq/1_kallisto/official/"
sample_id <- dir(file.path(dir))
sample_id <- sample_id[grep("^S", sample_id)] #remove the unrelated file if there is
...
for (i in 1:5) {
  ...
  write.table(sub, paste0("Files/GENIE3/kallisto_GENIE3/e7.5_", i+1, "_KK.tsv"), row.names = F, quote = F, sep = '\t')
}
...
for (i in 1:6) {
  ...
  write.table(sub, paste0("Files/GENIE3/kallisto_GENIE3/e8.5_", i, "_KK.tsv"), row.names = F, quote = F, sep = '\t')
}
...
for (i in 1:5) {
  ...
  write.table(sub, paste0("Files/GENIE3/kallisto_GENIE3/e9.5_", i, "_KK.tsv"), row.names = F, quote = F, sep = '\t')
}
```

```{r}
sink("2.1_sessionInfo.txt")
sessionInfo()
sink()
sessionInfo()
```