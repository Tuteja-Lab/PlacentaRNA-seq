...
allGenes <- read.table("Files/e7.5specific_ensGenes.txt", header = F)
allGenes <- unique(t2g[t2g$ens_gene %in% allGenes$V1, "ext_gene"])
...
ggplot(t2, aes(x = t2)) + geom_histogram(binwidth = 1, color="darkgreen", fill="#b8e186") +
    geom_vline(aes(xintercept = length(intersect(all, annot))), color="black", linetype="dashed", size=1) +
  ggtitle("e7.5 hub genes")
```

E8.5 genes in networks
```{r}
allGenes <- read.table("Files/e8.5specific_ensGenes.txt", header = F)
allGenes <- unique(t2g[t2g$ens_gene %in% allGenes$V1, "ext_gene"])
...
ggplot(t2, aes(x = t2)) + geom_histogram(binwidth = 1, color="darkblue", fill="lightblue") +
    geom_vline(aes(xintercept = length(intersect(all, annot))), color="black", linetype="dashed", size=1) +
...
```{r}
allGenes <- read.table("Files/e9.5specific_ensGenes.txt", header = F)
allGenes <- unique(t2g[t2g$ens_gene %in% allGenes$V1, "ext_gene"])
...
ggplot(t2, aes(x = t2)) + geom_histogram(binwidth = 1, alpha=.2, fill="#FF6666", color="#841F27") +
    geom_vline(aes(xintercept = length(intersect(all, annot))), color="black", linetype="dashed", size=1) +
...
result <- foreach(i=1:10000) %dopar% randFunc(unique(pop$ens_gene), length(sub$name), args[4], i)
stopCluster(cl)
...
Rscript --vanilla randomizationGO.R "/work/LAS/geetu-lab/hhvu/project1_2/rna-seq/PlacentaRNA-seq/Files/e7.5Group.txt" "/work/LAS/geetu-lab/hhvu/project1_2/rna-seq/PlacentaRNA-seq/Files/STRING/e7.5/largestComponent/e7.5_1/e7.5_1_nodeTable.csv" "/work/LAS/geetu-lab/hhvu/project1_2/rna-seq/randomizationGO/e7.5_1_STRING_inflammatory response_qval.txt" "inflammatory response"
```
```{r}
sessionInfo()
```